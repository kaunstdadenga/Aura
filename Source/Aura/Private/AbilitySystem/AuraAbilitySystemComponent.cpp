...
void UAuraAbilitySystemComponent::ForEachAbility(const FForEachAbility& Delegate) {
	// Mutex - Abilities can change so locking it prevents an error in this loop
...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag,
	...
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag)) {
		...
		const bool bStatusValid = Status == GameplayTags.Abilities_Status_Equipped || Status ==
			GameplayTags.Abilities_Status_Unlocked;
		if (bStatusValid) {
			// Remove this Input (slot) from Ability that has it
			ClearAbilitiesOfSlot(Slot);
			// Clear this Abilities Slot, just in case it is a different slot
